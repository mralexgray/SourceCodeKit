...
/*! SCKProject represents an IDE project that tracks several source files usually 
    put together in a directory. In addition, SCKProject provides access to the program 
    components declared in the project files. @see -classes, @see-functions and @see-globals.
    ...
    protocols on the behalf of SCKProject instance. 
    
    The collection returned by @c-content depends on the class set with @c-setContentClass:. */
    
@interface SCKProject : NSObject

/*! @brief  Initializes and returns a new project based on the directory URL (to resolve relative
            paths) and the provided source collection to retrieve the SCKFile objects.
    @note   A source collection can be shared between several projects (it caches SCKFile objects).
    @param  aSourceCollection if nil, raises a NSInvalidArgumentException. 
*/
...
@property (readonly) SCKSourceCollection *sourceCollection;

/*! Adds the file that corresponds to the URL to the project.
    @param aURL when nil, raises a NSInvalidArgumentException. */
...
/*! Array containing SCKFile objects of the files that belong to the project. */ 
...
/*! The content class that controls the current content exposed through the collection protocols.
...
@protocol SCKProjectContent <NSObject>  - (id)content; @end

/*! Content class to present the project files. */

@interface SCKFileBrowsingProjectContent : NSObject <SCKProjectContent> @end

/*! Content class to present the project program components grouped into classes,functions and globals. */

@interface SCKSymbolBrowsingProjectContent : NSObject <SCKProjectContent> @end
